...
def rename_group_files(raw_dir: Path, group: str, old_label: str, new_label: str, dry_run: bool):
    if len(new_label) != 4 or not new_label.isdigit():
        raise ValueError(f'New label must be a 4-digit string, got {new_label!r}')
...
def main():
    parser = argparse.ArgumentParser(description='Relabel one grouped OAuth captcha batch in data/oauthcaptcha.')
    parser.add_argument(
        'group_or_label',
        nargs='?',
        help='Group id, or the new label when targeting the latest group.',
    )
    parser.add_argument('new_label', nargs='?', help='Correct 4-digit label to write into filenames.')
    parser.add_argument('--raw-dir', default='data/oauthcaptcha', help='Directory containing the collected PNG files.')
    parser.add_argument(
        '--latest',
        '--edit-latest',
        dest='latest',
    ...
    parser.add_argument(
        '--old-label',
        help='Current 4-digit label to replace. Required only when the group has mixed labels.',
    )
    parser.add_argument(
        '--preview-index',
        type=int,
    ...
    if new_label is None:
        new_label = input('Enter the correct 4-digit label: ').strip()
